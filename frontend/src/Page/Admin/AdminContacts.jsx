...
const AdminContacts = () => {
  ...
  const handleStatusUpdate = async (newStatus) => {
    try {
      ...
      Swal.fire("오류 발생생", "수정 중 문제가 발생했습니다.", "error");
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    if(result.isConfirmed){
      try{
        ...
        Swal.fire("삭제 완료", "상태가 성공적으로 삭제되었습니다.", "success");
  ...
  return (
    <div className="p-4 mx-auto max-w-[1400px]">
        ...
        <>
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full bg-white shadow-ml rounded-lg overflow-hidden text-sm lg:text-lg font-bold">
              ...
              <tbody>
                {paginationContacts.map((contact, index) => (
      ...
      {isModalOpen && selectedContact && (
        <div className="fixed inset-0 bg-black opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                <button
                  ...
                  className="w-full px-4 py-2 bg-blue-800 rounded hover:bg-blue-200"
                >
                  대기중
                </button>
                <button
                  onClick={() => handleStatusUpdate("in progress")}
                  className="w-full px-4 py-2 bg-yellow-800 rounded hover:bg-yellow-200"
                >
                  진행중
                </button>
                <button
                  onClick={() => handleStatusUpdate("completed")}
                  className="w-full px-4 py-2 bg-green-800 rounded hover:bg-green-200"
                >
                  완료
                </button>
              </div>
            ...
            <div className="flex justify-end">
              <button
                ...
                className="w-full px-4 py-2 bg-gray-100 text-gray-800 rounded"
              >
                취소
              </button>
            </div>
